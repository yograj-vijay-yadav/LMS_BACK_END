...
var cassandra_exports = /* @__PURE__ */ require_runtime.__exportAll({
...
var CassandraClientFactory = class CassandraClientFactory {
	...
	*/
	static async getClient(args) {
		return new cassandra_driver.Client(await this.processArgs(args));
	}
	/**
	...
	*/
	static async processAstraArgs(args) {
		...
		if (astraArgs.endpoint) {
			const hostnameParts = new URL(astraArgs.endpoint.toString()).hostname.split("-");
	...
	*/
	static async getAstraDefaultBundleLocation(astraArgs) {
		...
		scbFileName += ".zip";
		return node_path.default.join(dir, scbFileName);
	}
	/**
	...
	*/
	static async setAstraBundle(astraArgs, scbPath) {
		if (typeof scbPath === "string") try {
			...
			if ((Date.now() - stats.mtime.getTime()) / (1e3 * 60 * 60 * 24) > 360) await CassandraClientFactory.downloadAstraSecureConnectBundle(astraArgs, scbPath);
		} catch (error) {
	...
	*/
	static async downloadAstraSecureConnectBundle(astraArgs, scbPath) {
		...
		const url = (astraArgs.bundleUrlTemplate ? astraArgs.bundleUrlTemplate : "https://api.astra.datastax.com/v2/databases/{database_id}/secureBundleURL?all=true").replace("{database_id}", astraArgs.datacenterID);
		...
		try {
			await node_fs_promises.default.unlink(scbPath);
		} catch {}
		const fileHandle = await node_fs_promises.default.open(scbPath, "wx", 384);
		try {
...
var CassandraTable = class {
	...
	*/
	constructor(args, client) {
		...
		this.asyncCaller = new _langchain_core_utils_async_caller.AsyncCaller(this.constructorArgs);
	...
	*/
	buildWhereClause(filters) {
		...
		return `WHERE ${filters.map(({ name, operator = "=", value }) => {
			const normalizedOperator = operator.toUpperCase();
			const valueArray = Array.isArray(value) ? value : [value];
			if (valueArray.length === 1 && normalizedOperator !== "IN") return `${name} ${operator} ?`;
			else {
				const nameQuestionMarkCount = (name.replace(/'[^']*'|"[^"]*"/g, "").match(/\?/g) || []).length;
	...
	*/
	buildOrderByClause(filters) {
		if (!filters || filters.length === 0) return "";
		return `ORDER BY ${filters.map(({ name, operator, value }) => {
	...
	*/
	buildSearchQuery(queryColumns, filters, orderBy, limit, allowFiltering) {
		...
		return `SELECT ${selectColumns} FROM ${this.keyspace}.${this.table} ${whereClause} ${orderByClause} ${limitClause} ${allowFilteringClause}`;
	}
	/**
	...
	*/
	async _upsert(values, columns) {
		if (values.length === 0) return;
		await this.initialize();
		const upsertColumnNames = this._columnCheck(values, columns).map((col) => col.name);
