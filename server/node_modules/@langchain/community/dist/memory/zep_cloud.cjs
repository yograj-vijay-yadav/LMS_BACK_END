...
const condenseZepMemoryIntoHumanMessage = (memory) => {
	...
	return new _langchain_core_messages.HumanMessage(`${systemPrompt}\n${concatMessages}`);
};
const zepMemoryToMessages = (memory) => {
	const systemPrompt = zepMemoryContextToSystemPrompt(memory);
	let messages = systemPrompt ? [new _langchain_core_messages.SystemMessage(systemPrompt)] : [];
	if (memory && memory.messages) messages = messages.concat(memory.messages.filter((m) => m.content).map((message) => {
		...
		if (roleType === "user") return new _langchain_core_messages.HumanMessage(messageContent);
		else if (role === "assistant") return new _langchain_core_messages.AIMessage(messageContent);
		else return new _langchain_core_messages.ChatMessage(messageContent, roleType ?? role);
	}));
	return messages;
};
/**
...
var ZepCloudMemory = class extends require_memory_chat_memory.chat_memory_exports.BaseChatMemory {
	...
	constructor(fields) {
		...
		this.zepClient = new _getzep_zep_cloud.ZepClient({ apiKey: this.apiKey });
	}
	get memoryKeys() {
	...
	*/
	async loadMemoryVariables(values) {
		...
		try {
			...
			if (error instanceof _getzep_zep_cloud_api_index_js.NotFoundError) return this.returnMessages ? { [this.memoryKey]: [] } : { [this.memoryKey]: "" };
			throw error;
		}
		...
		return { [this.memoryKey]: this.separateMessages ? (0, _langchain_core_messages.getBufferString)(zepMemoryToMessages(memory), this.humanPrefix, this.aiPrefix) : condenseZepMemoryIntoHumanMessage(memory).content };
	}
	/**
