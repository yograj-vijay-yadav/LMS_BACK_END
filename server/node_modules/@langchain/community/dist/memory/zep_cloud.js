...
import { NotFoundError } from "@getzep/zep-cloud/api/index.js";
//#region src/memory/zep_cloud.ts
var zep_cloud_exports = /* @__PURE__ */ __exportAll({
...
const zepMemoryToMessages = (memory) => {
	...
	if (memory && memory.messages) messages = messages.concat(memory.messages.filter((m) => m.content).map((message) => {
		...
		if (roleType === "user") return new HumanMessage(messageContent);
		else if (role === "assistant") return new AIMessage(messageContent);
		else return new ChatMessage(messageContent, roleType ?? role);
	}));
	return messages;
};
/**
...
* ```
*/
var ZepCloudMemory = class extends chat_memory_exports.BaseChatMemory {
	humanPrefix = "Human";
	aiPrefix = "AI";
	memoryKey = "history";
	apiKey;
	...
	constructor(fields) {
		...
		this.zepClient = new ZepClient({ apiKey: this.apiKey });
	}
	get memoryKeys() {
	...
	*/
	async loadMemoryVariables(values) {
		...
		return { [this.memoryKey]: this.separateMessages ? getBufferString(zepMemoryToMessages(memory), this.humanPrefix, this.aiPrefix) : condenseZepMemoryIntoHumanMessage(memory).content };
	}
	/**
...
//#endregion
export { ZepCloudMemory, condenseZepMemoryIntoHumanMessage, zepMemoryContextToSystemPrompt, zepMemoryToMessages, zep_cloud_exports };
