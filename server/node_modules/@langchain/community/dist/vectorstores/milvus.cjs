...
let _zilliz_milvus2_sdk_node = require("@zilliz/milvus2-sdk-node");
//#region src/vectorstores/milvus.ts
var milvus_exports = /* @__PURE__ */ require_runtime.__exportAll({ Milvus: () => Milvus });
...
* class.
*/
var Milvus = class Milvus extends _langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
			ssl: "MILVUS_SSL",
	...
	constructor(embeddings, args) {
		...
		this.metadataTextFieldMaxLength = args.metadataTextFieldMaxLength ?? MILVUS_METADATA_TEXT_FIELD_MAX_LENGTH;
		this.partitionKey = args.partitionKey;
		this.partitionKeyMaxLength = args.partitionKeyMaxLength ?? MILVUS_PARTITION_KEY_MAX_LENGTH;
		this.fields = [];
		const url = args.url ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("MILVUS_URL");
		...
		this.client = new _zilliz_milvus2_sdk_node.MilvusClient(clientConfig);
	}
	/**
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		if (insertResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error ${this.autoId ? "inserting" : "upserting"} data: ${JSON.stringify(insertResp)}`);
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		if (await this.hasCollection() === false) throw new Error(`Collection not found: ${this.collectionName}, please create collection before search.`);
		...
		if (loadResp.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error loading collection: ${loadResp}`);
		const outputFields = this.fields.filter((field) => field !== this.vectorField);
		const searchResp = await this.client.search({
			collection_name: this.collectionName,
			partition_names: this.partitionName ? [this.partitionName] : void 0,
			search_params: {
			...
			vector_type: _zilliz_milvus2_sdk_node.DataType.FloatVector,
			vectors: [query],
			filter: filterStr
		});
		if (searchResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error searching data: ${JSON.stringify(searchResp)}`);
		const results = [];
		searchResp.results.forEach((result) => {
			...
			results.push([new _langchain_core_documents.Document(fields), result.score]);
		});
		return results;
	}
	/**
	...
	*/
	async ensurePartition() {
		...
		if (hasPartResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error checking partition: ${JSON.stringify(hasPartResp, null, 2)}`);
	...
	* @returns Promise resolving to void.
	*/
	async createCollection(vectors, documents, properties) {
		const fieldList = [];
		fieldList.push(...createFieldTypeForMetadata(documents, this.primaryField, this.metadataTextFieldMaxLength, this.partitionKey));
		if (this.autoId) fieldList.push({
			name: this.primaryField,
			description: "Primary key",
			data_type: _zilliz_milvus2_sdk_node.DataType.Int64,
			is_primary_key: true,
			autoID: true
		});
		else fieldList.push({
			name: this.primaryField,
			description: "Primary key",
			data_type: _zilliz_milvus2_sdk_node.DataType.VarChar,
			is_primary_key: true,
			autoID: false,
			max_length: 65535
		});
		fieldList.push({
			...
			data_type: _zilliz_milvus2_sdk_node.DataType.FloatVector,
		...
		if (createRes.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Failed to create collection: ${JSON.stringify(createRes)}`);
		const extraParams = {
			...this.indexCreateParams,
	...
	*/
	async grabCollectionFields() {
		...
		(await this.client.describeCollection({ collection_name: this.collectionName })).schema.fields.forEach((field) => {
			...
			const dtype = _zilliz_milvus2_sdk_node.DataTypeMap[field.data_type];
			if (dtype === _zilliz_milvus2_sdk_node.DataType.FloatVector || dtype === _zilliz_milvus2_sdk_node.DataType.BinaryVector) this.vectorField = field.name;
			if (dtype === _zilliz_milvus2_sdk_node.DataType.VarChar && field.name === MILVUS_TEXT_FIELD_NAME) this.textField = field.name;
		});
	}
	async hasCollection() {
		const hasColResp = await this.client.hasCollection({ collection_name: this.collectionName });
		if (hasColResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error checking collection: ${JSON.stringify(hasColResp, null, 2)}`);
	...
	*/
	async setProperties(properties) {
		if (await this.hasCollection() === false) throw new Error(`Collection ${this.collectionName} does not exist.`);
		const nullKeys = Object.keys(properties).filter((key) => properties[key] === null);
		const nonNullProperties = Object.fromEntries(Object.entries(properties).filter(([, value]) => value !== null));
	...
	*/
	async delete(params) {
		if (await this.hasCollection() === false) throw new Error(`Collection not found: ${this.collectionName}, please create collection before deletion.`);
		const loadResp = await this.client.loadCollectionSync({ collection_name: this.collectionName });
		if (loadResp.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error loading collection: ${loadResp}`);
		...
		if (filter && !ids) {
			...
			if (deleteResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error deleting data: ${JSON.stringify(deleteResp)}`);
			...
			if (deleteResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error deleting data with ids: ${JSON.stringify(deleteResp)}`);
		}
	}
};
function createFieldTypeForMetadata(documents, primaryFieldName, metadataTextFieldMaxLength, partitionKey) {
	...
	for (const [key, value] of Object.entries(sampleMetadata)) {
		...
		if (type === "string" || type === "object" && value !== null) {
			...
			for (const { metadata } of documents) {
				const raw = type === "string" ? metadata[key] : JSON.stringify(metadata[key]);
	...
	for (const [key, value] of Object.entries(sampleMetadata)) {
		...
		if (key === primaryFieldName || key === partitionKey) {} else if (type === "string") fields.push({
			...
			data_type: _zilliz_milvus2_sdk_node.DataType.VarChar,
			type_params: { max_length: perFieldMaxLength[key].toString() }
		});
		else if (type === "number") fields.push({
			name: key,
			description: `Metadata Number field`,
			data_type: _zilliz_milvus2_sdk_node.DataType.Float
		});
		else if (type === "boolean") fields.push({
			name: key,
			description: `Metadata Boolean field`,
			data_type: _zilliz_milvus2_sdk_node.DataType.Bool
		});
