...
import { DataType, DataTypeMap, ErrorCode, MilvusClient } from "@zilliz/milvus2-sdk-node";
//#region src/vectorstores/milvus.ts
var milvus_exports = /* @__PURE__ */ __exportAll({ Milvus: () => Milvus });
...
* class.
*/
var Milvus = class Milvus extends VectorStore {
	get lc_secrets() {
		return {
			ssl: "MILVUS_SSL",
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		if (searchResp.status.error_code !== ErrorCode.SUCCESS) throw new Error(`Error searching data: ${JSON.stringify(searchResp)}`);
		const results = [];
	...
	*/
	async ensurePartition() {
		...
		if (hasPartResp.status.error_code !== ErrorCode.SUCCESS) throw new Error(`Error checking partition: ${JSON.stringify(hasPartResp, null, 2)}`);
	...
	*/
	async createCollection(vectors, documents, properties) {
		...
		if (createRes.error_code !== ErrorCode.SUCCESS) throw new Error(`Failed to create collection: ${JSON.stringify(createRes)}`);
		const extraParams = {
			...this.indexCreateParams,
	...
	*/
	async grabCollectionFields() {
		...
		(await this.client.describeCollection({ collection_name: this.collectionName })).schema.fields.forEach((field) => {
			...
			if (dtype === DataType.FloatVector || dtype === DataType.BinaryVector) this.vectorField = field.name;
			if (dtype === DataType.VarChar && field.name === MILVUS_TEXT_FIELD_NAME) this.textField = field.name;
		});
	}
	async hasCollection() {
		const hasColResp = await this.client.hasCollection({ collection_name: this.collectionName });
		if (hasColResp.status.error_code !== ErrorCode.SUCCESS) throw new Error(`Error checking collection: ${JSON.stringify(hasColResp, null, 2)}`);
	...
	*/
	async delete(params) {
		...
		if (filter && !ids) {
			...
			if (deleteResp.status.error_code !== ErrorCode.SUCCESS) throw new Error(`Error deleting data with ids: ${JSON.stringify(deleteResp)}`);
		}
	}
};
