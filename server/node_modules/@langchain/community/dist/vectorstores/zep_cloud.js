...
var zep_cloud_exports = /* @__PURE__ */ __exportAll({ ZepCloudVectorStore: () => ZepCloudVectorStore });
function zepDocsToDocumentsAndScore(results) {
	return results.map((d) => [new Document({
...
*/
var ZepCloudVectorStore = class ZepCloudVectorStore extends VectorStore {
	client;
	collectionName;
	initPromise;
	...
	*/
	async initCollection(args) {
		...
		else this.client = new ZepClient({ apiKey: args.apiKey });
		try {
			this.collectionName = args.collectionName;
			await this.client.document.getCollection(this.collectionName);
		} catch (err) {
			if (err instanceof Error) if (err instanceof NotFoundError || err.name === "NotFoundError") await this.createCollection(args);
			else throw err;
		}
	}
	/**
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return maximalMarginalRelevance(queryEmbedding, r.results.map((doc) => Array.from(doc.embedding ? doc.embedding : [])), lambda, k).filter((idx) => idx !== -1).map((idx) => results[idx][0]);
	}
...
//#endregion
export { ZepCloudVectorStore, zep_cloud_exports };
