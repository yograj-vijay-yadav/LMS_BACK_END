...
let _langchain_core_utils_testing = require("@langchain/core/utils/testing");
let _langchain_core_utils_math = require("@langchain/core/utils/math");
//#region src/vectorstores/zep_cloud.ts
var zep_cloud_exports = /* @__PURE__ */ require_runtime.__exportAll({ ZepCloudVectorStore: () => ZepCloudVectorStore });
function zepDocsToDocumentsAndScore(results) {
	return results.map((d) => [new _langchain_core_documents.Document({
...
*/
var ZepCloudVectorStore = class ZepCloudVectorStore extends _langchain_core_vectorstores.VectorStore {
	client;
	collectionName;
	initPromise;
	...
	*/
	async initCollection(args) {
		...
		else this.client = new _getzep_zep_cloud.ZepClient({ apiKey: args.apiKey });
		try {
			this.collectionName = args.collectionName;
			await this.client.document.getCollection(this.collectionName);
		} catch (err) {
			if (err instanceof Error) if (err instanceof _getzep_zep_cloud_api_index_js.NotFoundError || err.name === "NotFoundError") await this.createCollection(args);
			else throw err;
		}
	}
	/**
	...
	*/
	async similaritySearch(query, k = 4, filter = void 0, _callbacks = void 0) {
		...
		return zepDocsToDocumentsAndScore(zepResults).map((result) => result[0]);
	}
	/**
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, r.results.map((doc) => Array.from(doc.embedding ? doc.embedding : [])), lambda, k).filter((idx) => idx !== -1).map((idx) => results[idx][0]);
	}
	static async init(zepConfig) {
		const instance = new this(new _langchain_core_utils_testing.FakeEmbeddings(), zepConfig);
