...
* ```
*/
var ParentDocumentRetriever = class extends MultiVectorRetriever {
	static lc_name() {
	...
	async _getRelevantDocuments(query) {
		...
		const parentDocs = [];
		const retrievedDocs = (await this.docstore.mget(parentDocIds)).filter((doc) => doc !== void 0);
	...
	*/
	async addDocuments(docs, config) {
		...
		for (let i = 0; i < parentDocs.length; i += 1) {
			...
			const taggedSubDocs = (await this.childSplitter.splitDocuments([parentDoc], childDocChunkHeaderOptions)).map((subDoc) => new Document({
...
//#endregion
export { ParentDocumentRetriever, parent_document_exports };

//# sourceMappingURL=parent_document.js.map