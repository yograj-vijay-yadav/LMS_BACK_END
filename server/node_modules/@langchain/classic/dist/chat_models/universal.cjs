...
* @internal
*/
var ConfigurableModel = class ConfigurableModel extends _langchain_core_language_models_chat_models.BaseChatModel {
	_llmType() {
		return "chat_model";
	}
	...
	*/
	...
	async _generate(messages, options, runManager) {
		return (await this._model(options))._generate(messages, options ?? {}, runManager);
	}
	...
	*/
	...
	async stream(input, options) {
		const wrappedGenerator = new _langchain_core_utils_stream.AsyncGeneratorWithSetup({
			generator: await (await this._model(options)).stream(input, options),
			config: options
		});
...
exports.MODEL_PROVIDER_CONFIG = MODEL_PROVIDER_CONFIG;
exports._inferModelProvider = _inferModelProvider;
exports.getChatModelByClassName = getChatModelByClassName;
exports.initChatModel = initChatModel;
Object.defineProperty(exports, "universal_exports", {
	enumerable: true,
	get: function() {
		return universal_exports;
	}
});
