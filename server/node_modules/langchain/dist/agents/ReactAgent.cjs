...
var ReactAgent = class ReactAgent {
	...
	constructor(options, defaultConfig) {
		...
		*/
		...
		const { state, input, output } = require_annotation.createAgentState(this.options.responseFormat !== void 0 || hasDynamicStructuredResponse, this.options.stateSchema, this.options.middleware);
		const allNodeWorkflows = new _langchain_langgraph.StateGraph(state, {
			input,
			output,
		...
		this.#agentNode = new require_AgentNode.AgentNode({
			...
			systemMessage: require_utils.normalizeSystemPrompt(this.options.systemPrompt),
		...
		for (let i = 0; i < middleware.length; i++) {
			...
			if (m.beforeAgent) {
				beforeAgentNode = new require_BeforeAgentNode.BeforeAgentNode(m);
			...
			if (m.beforeModel) {
				beforeModelNode = new require_BeforeModelNode.BeforeModelNode(m);
			...
			if (m.afterModel) {
				afterModelNode = new require_AfterModelNode.AfterModelNode(m);
			...
			if (m.afterAgent) {
				afterAgentNode = new require_AfterAgentNode.AfterAgentNode(m);
		...
		* Add Nodes
		*/
		allNodeWorkflows.addNode(require_AgentNode.AGENT_NODE_NAME, this.#agentNode);
		/**
		...
		*/
		...
		const clientTools = toolClasses.filter(require_utils.isClientTool);
		/**
		...
		*/
		if (clientTools.length > 0 || hasWrapToolCallMiddleware) {
			const toolNode = new require_ToolNode.ToolNode(clientTools, {
				signal: this.options.signal,
				wrapToolCall: require_utils.wrapToolCall(middleware)
			});
			allNodeWorkflows.addNode(require_ToolNode.TOOLS_NODE_NAME, toolNode);
		}
		/**
		* Add Edges
		*/
		...
		else entryNode = require_AgentNode.AGENT_NODE_NAME;
		const loopEntryNode = beforeModelNodes.length > 0 ? beforeModelNodes[0].name : require_AgentNode.AGENT_NODE_NAME;
		const exitNode = afterAgentNodes.length > 0 ? afterAgentNodes[afterAgentNodes.length - 1].name : _langchain_langgraph.END;
		allNodeWorkflows.addEdge(_langchain_langgraph.START, entryNode);
		/**
		...
		*/
		...
		for (let i = 0; i < beforeAgentNodes.length; i++) {
			...
			if (node.allowed && node.allowed.length > 0) {
				const allowedMapped = node.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== "tools" || hasToolsAvailable);
				const destinations = Array.from(new Set([nextDefault, ...allowedMapped.map((dest) => dest === _langchain_langgraph.END ? exitNode : dest)]));
		...
		for (let i = 0; i < beforeModelNodes.length; i++) {
			...
			const current = node.name;
			const nextDefault = i === beforeModelNodes.length - 1 ? require_AgentNode.AGENT_NODE_NAME : beforeModelNodes[i + 1].name;
			if (node.allowed && node.allowed.length > 0) {
				const allowedMapped = node.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== "tools" || hasToolsAvailable);
		...
		if (afterModelNodes.length > 0 && lastAfterModelNode) allNodeWorkflows.addEdge(require_AgentNode.AGENT_NODE_NAME, lastAfterModelNode.name);
		else {
			const destinations = this.#getModelPaths(clientTools, false, hasToolsAvailable).map((p) => p === _langchain_langgraph.END ? exitNode : p);
			if (destinations.length === 1) allNodeWorkflows.addEdge(require_AgentNode.AGENT_NODE_NAME, destinations[0]);
			else allNodeWorkflows.addConditionalEdges(require_AgentNode.AGENT_NODE_NAME, this.#createModelRouter(exitNode), destinations);
		}
		...
		if (afterModelNodes.length > 0) {
			...
			const destinations = modelPaths.map((p) => p === _langchain_langgraph.END ? exitNode : p);
		...
		if (afterAgentNodes.length > 0) {
			...
			if (firstAfterAgent.allowed && firstAfterAgent.allowed.length > 0) {
				const allowedMapped = firstAfterAgent.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== "tools" || hasToolsAvailable);
				/**
				...
				*/
				const destinations = Array.from(new Set([_langchain_langgraph.END, ...allowedMapped]));
				allNodeWorkflows.addConditionalEdges(firstAfterAgentNode, this.#createAfterModelSequenceRouter(clientTools, firstAfterAgent.allowed, _langchain_langgraph.END, hasToolsAvailable), destinations);
			} else allNodeWorkflows.addEdge(firstAfterAgentNode, _langchain_langgraph.END);
		}
		/**
		...
		*/
		if (hasToolsAvailable) {
			...
			if (shouldReturnDirect.size > 0) allNodeWorkflows.addConditionalEdges(require_ToolNode.TOOLS_NODE_NAME, this.#createToolsRouter(shouldReturnDirect, exitNode, toolReturnTarget), [toolReturnTarget, exitNode]);
			else allNodeWorkflows.addEdge(require_ToolNode.TOOLS_NODE_NAME, toolReturnTarget);
		}
		/**
		* compile the graph
		*/
	...
	*/
	withConfig(config) {
		return new ReactAgent(this.options, (0, _langchain_core_runnables.mergeConfigs)(this.#defaultConfig, config));
	}
	/**
	...
	*/
	#getModelPaths(toolClasses, includeModelRequest = false, hasToolsAvailable = toolClasses.length > 0) {
		const paths = [];
		if (hasToolsAvailable) paths.push(require_ToolNode.TOOLS_NODE_NAME);
		if (includeModelRequest) paths.push(require_AgentNode.AGENT_NODE_NAME);
		paths.push(_langchain_langgraph.END);
		return paths;
	}
	/**
	...
	*/
	#createToolsRouter(shouldReturnDirect, exitNode, toolReturnTarget) {
		return (state) => {
			...
			if (_langchain_core_messages.ToolMessage.isInstance(lastMessage) && lastMessage.name && shouldReturnDirect.has(lastMessage.name)) return this.options.responseFormat ? toolReturnTarget : exitNode;
	...
	*/
	#createModelRouter(exitNode = _langchain_langgraph.END) {
		/**
	...
	#createAfterModelRouter(toolClasses, allowJump, exitNode, hasToolsAvailable = toolClasses.length > 0) {
		...
		return (state) => {
			...
			if (allowJump && builtInState.jumpTo) {
				const destination = require_utils$1.parseJumpToTarget(builtInState.jumpTo);
				if (destination === _langchain_langgraph.END) return exitNode;
				if (destination === "tools") {
					if (!hasToolsAvailable) return exitNode;
					return new _langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
						...state,
						jumpTo: void 0
					});
				}
				return new _langchain_langgraph.Send(require_AgentNode.AGENT_NODE_NAME, {
					...state,
					jumpTo: void 0
				});
			}
	...
	#createAfterModelSequenceRouter(toolClasses, allowed, nextDefault, hasToolsAvailable = toolClasses.length > 0) {
		...
		return (state) => {
			...
			if (builtInState.jumpTo) {
				const dest = require_utils$1.parseJumpToTarget(builtInState.jumpTo);
				if (dest === _langchain_langgraph.END && allowedSet.has(_langchain_langgraph.END)) return _langchain_langgraph.END;
				if (dest === "tools" && allowedSet.has("tools")) {
					if (!hasToolsAvailable) return _langchain_langgraph.END;
					return new _langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
						...state,
						jumpTo: void 0
					});
				}
				if (dest === "model_request" && allowedSet.has("model_request")) return new _langchain_langgraph.Send(require_AgentNode.AGENT_NODE_NAME, {
					...state,
					jumpTo: void 0
				});
			}
	...
	*/
	#createBeforeAgentRouter(toolClasses, nextDefault, exitNode, hasToolsAvailable = toolClasses.length > 0) {
		return (state) => {
			...
			const destination = require_utils$1.parseJumpToTarget(builtInState.jumpTo);
			if (destination === _langchain_langgraph.END)
 /**
	...
	*/
	#createBeforeModelRouter(toolClasses, nextDefault, hasToolsAvailable = toolClasses.length > 0) {
		return (state) => {
			...
			if (!builtInState.jumpTo) return nextDefault;
			const destination = require_utils$1.parseJumpToTarget(builtInState.jumpTo);
			if (destination === _langchain_langgraph.END) return _langchain_langgraph.END;
			if (destination === "tools") {
				if (!hasToolsAvailable) return _langchain_langgraph.END;
				return new _langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
					...state,
	...
	*/
	async #initializeMiddlewareStates(state, config) {
		if (!this.options.middleware || this.options.middleware.length === 0 || state instanceof _langchain_langgraph.Command || !state) return state;
	...
	*/
	async invoke(state, config) {
		const mergedConfig = (0, _langchain_core_runnables.mergeConfigs)(this.#defaultConfig, config);
