...
function createToolSelectionResponse(tools) {
	...
	const toolLiterals = tools.map((tool) => zod_v3.z.literal(tool.name));
	const toolEnum = zod_v3.z.union(toolLiterals);
	return zod_v3.z.object({ tools: zod_v3.z.array(toolEnum).describe("Tools to use. Place the most relevant tools first.") });
}
/**
...
//# sourceMappingURL=llmToolSelector.cjs.map