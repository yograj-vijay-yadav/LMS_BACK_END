...
var ClearToolUsesEdit = class {
	...
	constructor(config = {}) {
		...
		this.keep = require_summarization.keepSchema.parse(keep);
	...
	async apply(params) {
		...
		*/
		...
		for (let i = 0; i < messages.length; i++) {
			const msg = messages[i];
			if (_langchain_core_messages.ToolMessage.isInstance(msg)) candidates.push({
				idx: i,
				msg
			});
		}
		...
		*/
		...
		for (const { idx, msg: toolMessage } of initialCandidatesToClear) {
			...
			*/
			messages[idx] = new _langchain_core_messages.ToolMessage({
...
function contextEditingMiddleware(config = {}) {
	...
	return require_middleware.createMiddleware({
		...
		wrapModelCall: async (request, handler) => {
			...
			*/
			const systemMsg = request.systemPrompt ? [new _langchain_core_messages.SystemMessage(request.systemPrompt)] : [];
