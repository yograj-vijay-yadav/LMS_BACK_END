...
function parseMiddlewareState(stateSchema, state) {
	...
	if ((0, _langchain_core_utils_types.isInteropZodSchema)(stateSchema)) return (0, _langchain_core_utils_types.interopParse)(stateSchema, state);
...
function _addInlineAgentName(message) {
	if (!_langchain_core_messages.AIMessage.isInstance(message) || _langchain_core_messages.AIMessageChunk.isInstance(message)) return message;
...
function _removeInlineAgentName(message) {
	if (!_langchain_core_messages.AIMessage.isInstance(message) || !message.content) return message;
	let updatedContent = [];
	let updatedName;
...
function validateLLMHasNoBoundTools(llm) {
	...
	*/
	if (_langchain_core_runnables.RunnableSequence.isRunnableSequence(model)) model = model.steps.find((step) => _langchain_core_runnables.RunnableBinding.isRunnableBinding(step)) || model;
	/**
 ...
 /**
	...
	*/
	if (_langchain_core_runnables.RunnableBinding.isRunnableBinding(model)) {
...
function hasToolCalls(message) {
	return Boolean(_langchain_core_messages.AIMessage.isInstance(message) && message.tool_calls && message.tool_calls.length > 0);
}
/**
...
function normalizeSystemPrompt(systemPrompt) {
	if (systemPrompt == null) return new _langchain_core_messages.SystemMessage("");
	if (_langchain_core_messages.SystemMessage.isInstance(systemPrompt)) return systemPrompt;
	if (typeof systemPrompt === "string") return new _langchain_core_messages.SystemMessage({ content: [{
		type: "text",
		text: systemPrompt
	}] });
...
async function bindTools(llm, toolClasses, options = {}) {
	...
	if (_langchain_core_runnables.RunnableSequence.isRunnableSequence(llm)) {
		const modelStep = llm.steps.findIndex((step) => _langchain_core_runnables.RunnableBinding.isRunnableBinding(step) || require_model.isBaseChatModel(step) || require_model.isConfigurableModel(step));
		if (modelStep >= 0) {
		...
		const wrappedHandler = async (request, handler) => {
			...
			*/
			...
			try {
				...
				*/
				if (!_langchain_core_messages.ToolMessage.isInstance(result) && !(0, _langchain_langgraph.isCommand)(result)) throw new Error(`Invalid response from "wrapToolCall" in middleware "${m.name}": expected ToolMessage or Command, got ${typeof result}`);
				return result;
			} catch (error) {
				throw require_errors.MiddlewareError.wrap(error, m.name);
			}
		};
